...
class ElectronShowerShapeCorrector
{
public:
	ElectronShowerShapeCorrector();
	ElectronShowerShapeCorrector( const edm::ParameterSet &iConfig, GlobalVariablesComputer global_var );
        ~ElectronShowerShapeCorrector();
//        const static bool useXGB = true;
	pat::Electron corrector( pat::Electron &obj, CLHEP::HepRandomEngine& engine, double rho, noZS::EcalClusterLazyTools& LazyTool);
private:
	static vector<string> showerShapes_;
        const static bool useXGB = true;
//	edm::ConsumesCollector cc_;
//        GlobalVariablesComputer globalVariablesComputer_;
	std::map<string, MVAComputer<reco::GsfElectron, StringObjectFunction<reco::GsfElectron, true>, useXGB> > correctionsEB_;
        map<string, TFormula> correctionScalingsEB_;
        std::map<string, MVAComputer<reco::GsfElectron, StringObjectFunction<reco::GsfElectron, true>, useXGB> > correctionsEE_;
        map<string, TFormula> correctionScalingsEE_;
	bool correctShowerShapes_ = true;
};

vector<string> ElectronShowerShapeCorrector::showerShapes_ = {"r9", "s4", "sieie", "sieip", "etaWidth", "phiWidth"};

ElectronShowerShapeCorrector::ElectronShowerShapeCorrector( const edm::ParameterSet &iConfig, GlobalVariablesComputer global_var )
{
		...
		//---EB
                ...
                correctionsEB_[ss_var] = MVAComputer<reco::GsfElectron, StringObjectFunction<reco::GsfElectron, true>, useXGB>(xgb_config, &global_var);
		...
		//---EE
                ...
                correctionsEE_[ss_var] = MVAComputer<reco::GsfElectron, StringObjectFunction<reco::GsfElectron, true>, useXGB>(xgb_config, &global_var);
...
ElectronShowerShapeCorrector::~ElectronShowerShapeCorrector() {}

     	pat::Electron ElectronShowerShapeCorrector::corrector( pat::Electron &obj , CLHEP::HepRandomEngine& engine, double rho, noZS::EcalClusterLazyTools& lazyTool)
{
...
// || std::isinf(obj.pfPhoIso03()) || std::isinf(obj.pfChgIsoWrtChosenVtx03()) || std::isinf(obj.pfChgIsoWrtWorstVtx03())
))
        {
            if(correctShowerShapes_)
            {
//                reco::GsfElectron::ShowerShape correctedShowerShapes = obj.full5x5_showerShapeVariables();
...
typedef ElectronShowerShapeCorrector CorrectionSSforPhoton;     
#endif
